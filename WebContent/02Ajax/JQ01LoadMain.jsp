...
/*
  	...
  	1. $(선택자(셀렉터)).load()
  		: 외부의 문서를 동적으로 로드(포함)할 때 사용한다. load()사용시 파라미터가 필요없는 경우 생략이 가능하다
  		형식] $(셀렉터).load('파일경로', 파라미터); ex.$('#loadView').load('./common/01LoadSub.jsp');
...
//JS함수 내부에서 jQuery코드를 사용하는 것이 가능하다.
	...
	$(function(){
		...
		$('#loadParamBtn').click(function(){
			...
			//load()메소드 호출 시 두 번째 인자로 파라미터를 전달한다.
